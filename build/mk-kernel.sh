...
make -j$(nproc) modules_install INSTALL_MOD_PATH=${OUT}/rootfs/
make -j$(nproc) dtbs_install INSTALL_DTBS_PATH=${OUT}/kernel/
...
if [ "${ARCH}" == "arm" ]; then
	...
	cp ${LOCALPATH}/kernel/arch/arm64/boot/dts/rockchip/overlays-brainypi/hw_intfc.conf ${OUT}/kernel/
fi
